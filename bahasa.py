game_dict = {"en":{"game_title"         : "TOWER OF HANOI",
                   ...
                   "secret_text1"       : "There is a mathematical secret behind this game… It turns out you only need no more than 2n-1 moves to move n discs from the initial peg to the final peg. This optimum solution is based on a recursive process. That is, a process which is done repeatedly.\n\nBy knowing this process, you can solve this game much quicker! All you need to do is memorising this process (or as we like to call it, algorithm).\n\nTo make things easier, we label the peg A, B, and C, as follows.",
                   ...
                   "secret_text4"       : "So where does the 2n-1 come from?\n\nWell, let’s have a look at our algorithm. Let Hn be the number of moves needed to move n discs from an initial peg to target peg. Since there are n-1pegs to be moved in step 1, thus step 1 needs Hn-1moves. It is obvious that step 2 needs only 1 move.\n\nHow about step 3? For the same reason as step 1, it needs Hn-1moves. So, we can conclude that Hn = Hn-1 + Hn-1 + 1 = 2Hn-1 + 1. By solving this recurrence relation, you can find that Hn = 2n - 1, by setting H1 = 1.\n\nThis result can also be proved using induction.",
		   "citation_text"	: "Source:\n\nLiu, D. The Tower of Hanoi.\nRetrieved from http://www2.mtsd.k12.wi.us/homestead/users /ordinans/Tower%20of%20Hanoi.html. \n\nMG's Tower of Hanoi For Python.\nRetrieved from https://www.pygame.org/project/3191"},
             ...
             "id":{"game_title"         : "MENARA HANOI",
                   ...
                   "secret_text4"       : "Lalu, dari mana kita tahu bahwa kita memerlukan 2n-1 langkah untuk memindahkan n keping?\n\nHasil tersebut bisa didapat dengan memperhatikan algoritma yang digunakan. Misal Hn adalah langkah yang diperlukan untuk memindahkan n keping dari tiang awal ke tiang tujuan. Sebab kita ingin memindahkan n-1 keping pada langkah 1, banyaknya langkah yang diperlukan untuk melakukan langkah 1 adalah Hn-1. Jelas bahwa kita perlu hanya 1 langkah untuk melakukan langkah 2.\n\nDengan alasan yang serupa dengan langkah 1, kita memerlukan Hn-1 langkah untuk melakukan langkah 3 sehingga kita mendapatkan hubungan rekursif Hn = Hn-1 + Hn-1 + 1 = 2Hn-1 + 1.\n\nDengan menginisialisasi nilai H1 = 1, bisa didapat bahwa Hn = 2n-1. Hasil ini juga dapat dibuktikan menggunakan induksi.",
